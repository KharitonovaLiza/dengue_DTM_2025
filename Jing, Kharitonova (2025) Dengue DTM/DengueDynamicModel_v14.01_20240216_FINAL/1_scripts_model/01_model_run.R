  ...
  for(sim_nb in 1:nb_sim){
    ...
    run_success[sim_nb]<-tryCatch({  
      ...
      if(parms_gen$vac_switch && parms_gen$run_epi){                                                                            
        parms_vac_strat<-ParmsVacStrat_Load(path_inputs_vac_strat, parms_gen$set_vac_strat, parms_epi, parms_gen$timeframe)
      ...
      if(parms_gen$run_epi){
        ...
        if(parms_gen$vac_switch){
          ...
          vac_cohorts_nb<-parms_mod$parms_vac_strat$trac_vac_cohorts_nb                                                #Total nb of cohorts that will be vaccinated during the simulation
          ...
          vac_cohorts_age_at_vac <-parms_mod$parms_vac_strat$trac_vac_cohorts_age_at_vac
          vac_cohorts_year_of_vac<-parms_mod$parms_vac_strat$trac_vac_cohorts_year_of_vac
        ...
        for(year in 1:parms_gen$timeframe){
          ...
          if(parms_gen$vac_switch){
            ...
            #Update trace with new vaccinated in already vaccinated cohort
            
            if(year<parms_gen$timeframe && any(parms_mod$parms_vac_strat$vac_cohorts[1:100,year]< parms_mod$parms_vac_strat$vac_cohorts[2:101,year+1])){
              ...
              #Update trace in case of repeated vaccination in the same cohort
              trace<-Model_UpdateTrace(trace=trace,                                     #5D object that needs to be filled
                                       year=year+1,                                     #Nb of the currently simulated year (the trace is filled at the end of the year for the next simulation year)
                                       vac_cohorts_nb=vac_cohorts_nb,                   #Nb of vaccinated cohorts,
                                       vac_cohorts_age_at_vac=vac_cohorts_age_at_vac,   #Vector indicating age at vaccination, for each vaccinated cohort
                                       vac_cohorts_year_of_vac=vac_cohorts_year_of_vac, #Vector indicating year of vaccination (as simulation year), for each vaccinated cohort  
                                       vac_levels=vac_levels,                           #Nb of vaccination statuses
                                       parms_mod=parms_mod,                             #List of model parameters,
                                       yini_6D=yini_6D)                                 #Array with the initial state for the hosts for the current year (post-ageing, post-vaccination)
            }
          ...
          if(year==parms_gen$timeframe && parms_gen$vac_switch){
            ...
            for(vac_cohort in 1:vac_cohorts_nb){                          #Loop through each vaccinated cohort
              ...
              year_row<-1
              for(y in year_of_vac:(min(101-age_at_vac_ind+1,parms_gen$timeframe))){                  #Loop through all the years that cohort was vaccinated
...
Model_FillTrace<-function(trace,                    #5D object that needs to be filled
                          ...
                          year,                     #Nb of the currently simulated year (the trace is filled at the end of the year for the next simulation year)
  ...
  for(vac_cohort in 1:vac_cohorts_nb){
    ...
    #================================================
    if(year_of_vac<=year && age_current_yrs<101){                                   #Year of vaccination is smaller than or equal to the current year --> cohort already vaccinated --> proceed (otherwise this cohort is ignored for this year and all values for it remain zero)
      ...
      if(parms_mod$parms_vac_strat$vac_strat_scope==1){       #If all individuals are vaccinated without testing then estimate object to extra check the calculations
        ...
        coverage<-parms_mod$parms_vac_strat$vac_cohorts[age_current_ind, year]                #Coverage at which this cohort is vaccinated
      ...
      for(day in 2:366){  
        ...
        if(year<timeframe){        #At each year the row of the trace is filled that corresponds to the NEXT year. Therefore, if the last simulation year, not filling the trace (as already in the last row of the object)
          ...
          for(inf_nb in 1:4){         #Loop through all new infections (1st to 4th)
            ...
            if(pop_at_risk>0){
              ...
              if(round(weight_total,10)!=1){
                ...
                message(weight_total)
              }
            }
          }
          
          #-- Check trace pop counts before ageing ---   #If no testing prior to vaccination
          if(parms_mod$parms_vac_strat$vac_strat_scope==1){
          ...
          if(min(trace[vac_cohort,,year:(year+1),,])<0){
            message("WARNING! Year ", year, "; day ", day, "; cohort ", vac_cohort, "; there are NEGATIVE VALUES IN THE TRACE. Smallest values is equal to ", min(trace[vac_cohort,,year:(year+1),,]))
          ...
          if(day==366){
            ...
            if(parms_mod$parms_vac_strat$vac_strat_scope==1){       #If all individuals are vaccinated without testing
...
############################################################
#This function updates the "Markov trace" with new vaccinated in already vaccinated cohort 
Model_UpdateTrace<-function(trace,                    #5D object that needs to be filled
                            year,                     #Nb of the currently simulated year (the trace is filled at the end of the year for the next simulation year)
                            vac_cohorts_nb,           #Nb of vaccinated cohorts,
                            vac_cohorts_age_at_vac,   #Vector indicating age at vaccination, for each vaccinated cohort
                            vac_cohorts_year_of_vac,  #Vector indicating year of vaccination (as simulation year), for each vaccinated cohort  
  ...
  for(vac_cohort in 1:vac_cohorts_nb){
    ...
    if(year_of_vac<=year && age_current_yrs<101){                                   #Year of vaccination is smaller than or equal to the current year --> cohort already vaccinated --> proceed (otherwise this cohort is ignored for this year and all values for it remain zero)
      ...
      #Determine status of re-vaccination in the cohort
      if(age_current_ind>1 && year>1){
        if(parms_mod$parms_vac_strat$vac_cohorts[age_current_ind-1, year-1]< parms_mod$parms_vac_strat$vac_cohorts[age_current_ind, year]  ){
          
          #Estimate nb of hosts with each status in the initial state. To do so, from the initial state for the current year (object "yini_6D") we take:
          #- Age group that corresponds to the current vaccinated cohort (DIMENSION 1)
          #- Combination(s) of indices for the four serotypes that correnspond(s) to each number of infections before vaccination (DIMENSIONS 2-5; here -1 indicates "not equal to 1", i.e. excluding the state "Susceptible to a serotype")
          #- Vaccinated individuals only (DIMENSION 6), as the probability of an infection in vaccinees can be different if the vaccine protects against infection
          
          x0_was<- sum(yini_6D[age_current_ind, 1, 1, 1, 1,(2:vac_levels)])     #History of no infections (index for each serotype is 1)
          
          x1_was<-(sum(yini_6D[ age_current_ind,-1, 1, 1, 1,(2:vac_levels)])+   #Hosts with the history of DENV-1 (index for DENV-1 is not 1),
                     sum(yini_6D[ age_current_ind, 1,-1, 1, 1,(2:vac_levels)])+ #Hosts with the history of DENV-2 (index for DENV-2 is not 1),
                     sum(yini_6D[ age_current_ind, 1, 1,-1, 1,(2:vac_levels)])+ #Hosts with the history of DENV-3 (index for DENV-3 is not 1),
                     sum(yini_6D[ age_current_ind, 1, 1, 1,-1,(2:vac_levels)])) #Hosts with the history of DENV-4 (index for DENV-4 is not 1)
          
          x2_was<-(sum(yini_6D[ age_current_ind,-1,-1, 1, 1,(2:vac_levels)])+   #All possible combinations of serotypes that result in two previous infections; here DENV-1 & DENV-2
                     sum(yini_6D[ age_current_ind,-1, 1,-1, 1,(2:vac_levels)])+ #... DENV-1 & DENV-3
                     sum(yini_6D[ age_current_ind,-1, 1, 1,-1,(2:vac_levels)])+ #... DENV-1 & DENV-4
                     sum(yini_6D[ age_current_ind, 1,-1,-1, 1,(2:vac_levels)])+ #... DENV-2 & DENV-3
                     sum(yini_6D[ age_current_ind, 1,-1, 1,-1,(2:vac_levels)])+ #... DENV-2 & DENV-4
                     sum(yini_6D[ age_current_ind, 1, 1,-1,-1,(2:vac_levels)])) #... DENV-3 & DENV-4
          
          x3_was<-(sum(yini_6D[ age_current_ind,-1,-1,-1, 1,(2:vac_levels)])+ #All combinations of serotypes that result in three previous infections (same logic as above)
                     sum(yini_6D[ age_current_ind,-1,-1, 1,-1,(2:vac_levels)])+ 
                     sum(yini_6D[ age_current_ind,-1, 1,-1,-1,(2:vac_levels)])+ 
                     sum(yini_6D[ age_current_ind, 1,-1,-1,-1,(2:vac_levels)])) 
          
          x4_was<- sum(yini_6D[ age_current_ind,-1,-1,-1,-1,(2:vac_levels)]) #History of four infections (index for each serotype is not 1)
          
          #if cohort was re-vaccinated, then new vaccinated occur 
          x0_new_vac<-x0_was-sum(trace[vac_cohort,1,year,1,])   # Cohort size with no infection - trace size without infection is the size of new vaccinated without infection
          x1_new_vac<-x1_was-sum(trace[vac_cohort,1:2,year,2,]) # Cohort size with 1 infection - trace size without infection is the size of new vaccinated without infection
          x2_new_vac<-x2_was-sum(trace[vac_cohort,1:3,year,3,]) # Cohort size with 2 infections - trace size without infection is the size of new vaccinated without infection
          x3_new_vac<-x3_was-sum(trace[vac_cohort,1:4,year,4,]) # Cohort size with 3 infections - trace size without infection is the size of new vaccinated without infection
          x4_new_vac<-x4_was-sum(trace[vac_cohort,1:5,year,5,]) # Cohort size with 4 infections - trace size without infection is the size of new vaccinated without infection
          
          trace[vac_cohort,1,year,1,1]<-trace[vac_cohort,1,year,1,1]+x0_new_vac
          trace[vac_cohort,2,year,2,1]<-trace[vac_cohort,2,year,2,1]+x1_new_vac
          trace[vac_cohort,3,year,3,1]<-trace[vac_cohort,3,year,3,1]+x2_new_vac
          trace[vac_cohort,4,year,4,1]<-trace[vac_cohort,4,year,4,1]+x3_new_vac
          trace[vac_cohort,5,year,5,1]<-trace[vac_cohort,5,year,5,1]+x4_new_vac
