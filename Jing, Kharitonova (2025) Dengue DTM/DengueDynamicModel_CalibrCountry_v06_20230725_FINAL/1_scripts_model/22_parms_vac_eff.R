...
# - ParmsVacEff_GetVacCohorts - This function creates a matrix indicating, for each year of age and each simulation year, whether this cohort is vaccinated or not
...
ParmsVacEff_Select<-function(parms_vac_eff_imported){   #Inputs imported from the Excel file Vac_XXXXX (output of the function Parms_Import)
  ...
  with(parms_vac_eff_imported,{ 
    ...
    parms_vac_eff<-list(
      ...
      vac_as_silent_inf=as.logical(vac_as_silent_inf),                        #Vaccination acts as a silent infection (required in the model interface) --> Boolean
...
#=========================================================================================================================================================================================================
#=========================================================================================================================================================================================================
#This function creates a matrix indicating, for each year of age and each simulation year, whether this cohort is vaccinated or not

ParmsVacEff_GetVacCohorts<-function(parms_vac_strat,                #Vaccination strategy parameters
                                    timeframe){                     #Simulation timeframe

  with(parms_vac_strat, {                          
    
    #Identify all vaccinated cohorts at each year of vaccination campaign
    vac_cohorts<-matrix(0,101,timeframe)                 #A flag for each cohort (total of 101 cohorts aged 0 to 100 years) at each year of vaccination campaign:
    #                                                       =0: Unvaccinated
    #                                                       =1: Vaccinated as part of routine vaccination (at any year)
    #                                                       =2: Vaccinated as part of catch-up vaccination (at year 1)
    
    for(y in 1:timeframe){                                              #Loop through each year of the timeframe/vaccination campaign
      cohort_routine_lower<-age_routine+1                               #The nb of the youngest cohort vaccinated as part of routine vaccination (always the one vaccinated this year; +1 because the age starts from 0 and indexing starts from 1)
      cohort_routine_upper<-min(101,(age_routine+y))                    #The nb of the oldest cohort vaccinated as part of routine vaccination (e.g. if routine vaccination takes place at 4 years of age, at year 1 the nb of the oldest cohort = 5 (4-year-olds), at year 2 nb = 6 (5-year-olds etc.)); truncated at 101 years
      
      vac_cohorts[cohort_routine_lower:cohort_routine_upper,y]<-1       #Flag=1 for the cohorts vaccinated as part of routine vaccination      
      
      if(catchup_switch){                                               #If catch-up vaccination is included,
        cohort_catchup_lower<-age_catchup_low+y                         #The nb of the youngest cohort vaccinated as part of catch-up vaccination at year 1
        cohort_catchup_upper<-min(101,(age_catchup_high+y))             #The nb of the oldest cohort vaccinated as part of catch-up vaccination at year 1 (truncated at 101 years)
        
        if(cohort_catchup_lower<=101){                                  #If the youngest cohort vaccinated as part of catch-up vaccination at year 1 is still alive
          vac_cohorts[cohort_catchup_lower:cohort_catchup_upper,y]<-2   #Flag=2 for the cohorts vaccinated as part of catch-up vaccination
        }
      }
    }
    
    #Note: For all other cohorts the value remains zero at each simulation year as they are never vaccinated.
...
ParmsVacEff_Initialise<- function(parms_vac_eff_selected,     #Selected and pre-formatted vaccine effects inputs
  ...
  #Matrix indicating whether a cohort is vaccinated or not
  vac_cohorts<-ParmsVacEff_GetVacCohorts(parms_vac_strat=parms_vac_strat$parms_vac_strat,       #Note: a bit of an awkward notation (list in the list), but does not affect functioning
                                         timeframe=timeframe)
...
ParmsVacEff_GetThetaAndPsyYear<-function(year,                     #Current simulation year
                                         ...
                                         vac_cohorts_year_of_vac   #Vector indicating calendar year of vaccination, for each vaccinated cohort  
){
  ...
  #For that we use the previously created matrix vac_cohorts

  #-----------------------------------
  #--- Initialize the objects --------
  #-----------------------------------
  eff_asympt        <-array(NA, dim=c(101,4,2)) #3D arrays with efficacy/effectiveness values. Dimensions are age (single year), serotype, serostatus at vaccination; for the currently simulated year
  eff_sympt_non_hosp<-array(NA, dim=c(101,4,2)) #The values represent efficacy (if boosting is not included) or effectiveness (if boosting is included)
  eff_sympt_hosp    <-array(NA, dim=c(101,4,2))
  eff_inf           <-array(NA, dim=c(101,4,2)) #Additional object with the efficacy/effectiveness against infection, which will be calculated from the efficacy against different types of dengue (asympt, sympt non-hosp, sympt hosp)
  ...
  for(age_ind in 1:101){                                      
    ...
    #If the cohort aged age_ind is not vaccinated in the current simulation year (flag value in vac_cohorts is 0), then zero efficacy for all serotypes and serostatuses
    if(parms_vac_eff$vac_cohorts[age_ind,year]==0){       
    ...
    #If cohort is vaccinated, continue
      ...
      #If vaccinated as part of routine vaccination (flag value in vac_cohorts is 1)
      if(parms_vac_eff$vac_cohorts[age_ind,year]==1){
        year_since_vac<-age_ind-parms_vac_strat$age_routine   #...year since vaccination is the difference between current age and the age at which routine vaccination takes place (e.g. someone vaccinated at 4yo will be 6 at year 3; age_ind for them is 7 (6+1); 7-4=3 --> CORRECT)
        age_at_vac<-parms_vac_strat$age_routine               #...age at vaccination is the age of routine vaccination
        year_of_vac<-year-(age_ind-age_at_vac)+1              #...year of vaccination can be estimated from the difference between the current age and age at vaccination
      }   
      
      #If vaccinated as part of catch-up campaign (flag value in vac_cohorts is 2)
      if(parms_vac_eff$vac_cohorts[age_ind,year]==2){
        year_since_vac<-year                                  #...year since vaccination is equal to the current year (as catch-up only takes place at year 1)
        year_of_vac<-1                                        #...year of vaccination is always equal to 1 
        age_at_vac<-age_ind-year                              #...age at year 1 can be calculated
      }
      ...
      #If this is NOT THE FIRST YEAR SINCE THE COHORT'S VACCINATION
        ...
        if(!parms_vac_eff$boosting_asymptomatic_switch){       #If boosting is not included, the efficacy depends on the time since vaccination
          eff_asympt[age_ind,,]<-parms_vac_eff$eff_asympt[,,year_since_vac, drop=FALSE]
        ...
        if(!parms_vac_eff$boosting_symptomatic_switch){       
          eff_sympt_non_hosp[age_ind,,]<-parms_vac_eff$eff_sympt_non_hosp[,,year_since_vac, drop=FALSE]    
          eff_sympt_hosp    [age_ind,,]<-parms_vac_eff$eff_sympt_hosp    [,,year_since_vac, drop=FALSE]  
        }else{                                                 
...
ParmsVacEff_GetSevDistr<-function(parms_epi,       #Population & transmission parameters 
  ...
  with(c(parms_epi,
         efficacies),{
           ...
           prop_sympt_hosp_unvac<-array(NA, dim=c(N_age_groups,4,3))                        #Dimensions are age & infection type & serotype
           ...
           #Therefore, not multiplying by the proportion of symptomatic in unvaccinated. These values, thus, represent % severe out of symptomatic infection (not overall infections)
